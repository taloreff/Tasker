...
export class LabelService {
  ...
  async create(createLabelDto: CreateLabelDto, userId: string): Promise<Label> {
    this.logger.log(
      `Creating label: ${createLabelDto.name} in workspace: ${createLabelDto.workspaceId} by user: ${userId}`
    );
    ...
    if (existingLabel) {
      throw new ConflictException(
        `Label "${createLabelDto.name}" already exists in this workspace`
      );
    }
  ...
  async findAllByWorkspace(
    ...
    this.logger.log(
      `Finding labels in workspace: ${workspaceId} for user: ${userId}`
    );
    ...
    this.logger.log(
      `Found ${labels.length} labels in workspace: ${workspaceId}`
    );
    return labels;
  }
  ...
  async update(
    ...
    if (updateLabelDto.name && updateLabelDto.name !== label.name) {
      ...
      if (existingLabel) {
        throw new ConflictException(
          `Label "${updateLabelDto.name}" already exists in this workspace`
        );
      }
    }
  ...
  async assignLabelsToTask(
    taskId: string,
    assignLabelsDto: AssignLabelsDto,
    userId: string
  ): Promise<void> {
    ...
    const invalidLabels = labels.filter(
      label => label.workspaceId !== taskWorkspaceId
    );
    if (invalidLabels.length > 0) {
      throw new BadRequestException(
        "Some labels do not belong to the task's workspace"
      );
    }
  ...
  async createDefaultLabels(
    ...
    this.logger.log(
      `Created ${createdLabels.length} default labels for workspace: ${workspaceId}`
    );
