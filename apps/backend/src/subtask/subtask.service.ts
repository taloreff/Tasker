...
export class SubtaskService {
  ...
  async create(
    ...
    this.logger.log(
      `Creating subtask: ${createSubtaskDto.name ||
        createSubtaskDto.title} for task: ${createSubtaskDto.itemId ||
        createSubtaskDto.taskId} by user: ${userId}`
    );
  ...
  async findAllByTask(itemId: string, userId: string): Promise<Subtask[]> {
  ...
  async findOne(id: string, userId: string): Promise<Subtask> {
    ...
    this.logger.log(`Subtask found: ${subtask.name} (ID: ${subtask.id})`);
    return subtask;
  }
  ...
  async toggleComplete(id: string, userId: string): Promise<Subtask> {
    this.logger.log(
      `Toggling completion status for subtask: ${id} by user: ${userId}`
    );
    ...
    const updatedSubtask = await this.subtaskRepo.save(subtask);
    this.logger.log(
      `Subtask completion toggled: ${updatedSubtask.id} - completed: ${updatedSubtask.completed}`
    );
  ...
  async reorderSubtasks(
    ...
    this.logger.log(
      `Reordering subtasks for task: ${itemId} by user: ${userId}`
    );
    ...
    if (invalidIds.length > 0) {
      throw new BadRequestException(
        `Some subtasks do not belong to task ${itemId}: ${invalidIds.join(
          ', '
        )}`
      );
    }
    ...
    const updatedSubtasks = await this.findAllByTask(itemId, userId);
    this.logger.log(`Subtasks reordered for task: ${itemId}`);
  ...
  async findUserSubtasks(userId: string): Promise<Subtask[]> {
    ...
    const subtasks = await this.subtaskRepo
      ...
      .where(
        '(subtask.createdById = :userId OR subtask.assignedToId = :userId)',
        { userId }
      )
